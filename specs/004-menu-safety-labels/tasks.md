...
- [x] T004 Extend parsing, formatting, manual create/update, atomic import replacement, and existing menu/item SSE payloads in `src/server/services/menu.ts` so omitted create/import lists become empty, omitted update lists are preserved, and invalid import values produce path-specific violations.
- [x] T005 Extend the existing thin payload forwarding in `src/server/routes/menus.ts` and typed item requests in `src/client/api.ts` for `allergens` and `additives` without adding endpoints.
