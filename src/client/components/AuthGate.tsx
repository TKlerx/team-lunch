...
function syncAuthenticatedStorage(
  user: NonNullable<AuthConfigResponse["auth"]["user"]>,
  role: AuthConfigResponse["auth"]["role"],
) {
  localStorage.setItem(ACTOR_KEY_STORAGE_KEY, user.username);
  if (user.displayName) {
    localStorage.setItem(DISPLAY_NAME_STORAGE_KEY, user.displayName);
  } else {
    localStorage.removeItem(DISPLAY_NAME_STORAGE_KEY);
  }
  localStorage.setItem(AUTH_METHOD_STORAGE_KEY, user.method);
  if (role) {
...
function DatabaseUnavailableScreen({ message }: { message: string }) {
  return (
    <div className="flex min-h-screen items-center justify-center bg-surface-muted p-4">
      <Card className="w-full max-w-xl border-danger p-6">
        ...
        <p className="text-sm text-fg-muted">
          {message ||
            "The database is unavailable. Start the database and reload this page."}
        </p>
      </Card>
    </div>
  );
...
function SetupRequiredScreen({ message }: { message: string }) {
  return (
    <div className="flex min-h-screen items-center justify-center bg-surface-muted p-4">
      <Card className="w-full max-w-xl border-danger p-6">
        <h2 className="mb-2 text-lg font-semibold text-fg">
        ...
        <p className="text-sm text-fg-muted">
          Configure Microsoft Entra sign-in or create DB-managed local accounts
          before using Team Lunch.
        </p>
        {message && (
...
function BlockedScreen({ username }: { username?: string | null }) {
  return (
    <div className="flex min-h-screen items-center justify-center bg-surface-muted p-4">
      <Card className="w-full max-w-xl border-danger p-6">
        <h2 className="mb-2 text-lg font-semibold text-fg">Access blocked</h2>
        <p className="mb-4 text-sm text-fg-muted">
          Your account has been blocked by an administrator. Contact the app
...
function ApprovalPendingScreen({ officeName }: { officeName?: string | null }) {
  return (
    <div className="flex min-h-screen items-center justify-center bg-surface-muted p-4">
      <Card className="w-full max-w-xl p-6">
        <h2 className="mb-2 text-lg font-semibold text-fg">Welcome</h2>
        <p className="mb-4 text-sm text-fg-muted">
          Your account is awaiting admin approval. Please contact your lunch app
          administrator.
        </p>
        <Panel>
...
interface SignInScreenProps {
  ...
  username: string;
  password: string;
  onLocalLogin: (event: FormEvent) => void;
...
export default function AuthGate({ children }: AuthGateProps) {
    ...
    void (async () => {
      ...
      try {
        ...
        if (payload.auth.authenticated && payload.auth.user) {
          syncAuthenticatedStorage(payload.auth.user, payload.auth.role);
        } else {
          localStorage.removeItem(AUTH_ROLE_STORAGE_KEY);
        }
  ...
  if (config?.databaseUnavailable) {
    return <DatabaseUnavailableScreen message={authWarning} />;
  }

  const authAvailable =
    !!config && (config.entraEnabled || config.localEnabled);

  if (!authAvailable) {
    return <SetupRequiredScreen message={authWarning} />;
  }

  if (config?.authenticated && config.blocked) {
    return <BlockedScreen username={config.user?.username} />;
  }
  ...
  if (
    ...
    return <ApprovalPendingScreen officeName={config.officeLocation?.name} />;
  }
      ...
      <AdminOfficeProvider
        ...
        officeLocations={
          config.isAdmin
            ...
            : config.accessibleOfficeLocations
        }
        pendingApprovalCount={
          config.isAdmin ? config.pendingApprovals.length : 0
        }
      >
        {children}
      </AdminOfficeProvider>
