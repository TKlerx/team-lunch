      ...
      try {
        ...
        if (payload.auth.authenticated && payload.auth.user) {
          localStorage.setItem(
            ACTOR_KEY_STORAGE_KEY,
            payload.auth.user.username,
          );
          if (payload.auth.user.displayName) {
            localStorage.setItem(
              DISPLAY_NAME_STORAGE_KEY,
              payload.auth.user.displayName,
            );
          } else {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-surface-muted p-4">
        <Card className="w-full max-w-xl border-danger p-6">
          ...
          <p className="text-sm text-fg-muted">
            Configure Microsoft Entra sign-in or create DB-managed local
            accounts before using Team Lunch.
          </p>
          {authWarning && (
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-surface-muted p-4">
        <Card className="w-full max-w-xl p-6">
          ...
          <p className="mb-4 text-sm text-fg-muted">
            Your account is awaiting admin approval. Please contact your lunch
            app administrator.
          </p>
          <Panel>
