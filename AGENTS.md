...
- `validate.ps1 all` no longer runs continuity freshness; use `./validate.ps1 continuity` explicitly when refreshing continuity snapshots.
- Continuity snapshots are generated by `npm run continuity:update` into `specs/CURRENT-WORK.md` and `specs/RECONCILIATION.md`; commit those files when you intentionally refresh continuity.
