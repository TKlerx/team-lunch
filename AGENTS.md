...
./validate.ps1 precommit    # pre-commit default: typecheck + lint only
...
- `validate.ps1 all` can fall back to generating a Prisma client without the query engine if a long-lived dev server is still holding the Windows engine DLL lock; stop the live `tsx watch src/server/index.ts` process before full validation so server tests run against the normal engine-backed client.
...
- `validate.ps1 precommit` is the commit-time fast gate (`typecheck` + `lint`); `validate.ps1 all` remains the local/CI quality gate, and continuity is still explicit/manual.
