...
- Side dishes are course-tagged separately from flavor tags (`course:side`); safe, explore, and pre-vote recommendations filter those items out as primary candidates, but their orders/ratings remain in history so they can still inform flavor preferences. The filter is intentionally conservative and falls back to the original list if every candidate is side-tagged.
