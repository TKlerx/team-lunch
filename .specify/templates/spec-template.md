...
<!--
  Team Lunch syncs all browsers via SSE (src/server/sse.ts → useSSE). Any state
  change a user must see in real time requires a broadcast event. List them here.
-->

- **[event-name]**: emitted when [state change]; payload contains [fields]; UI reaction: [what clients do]
...
- Multi-office aware: [yes/no — does this respect office context?]
- Auth scope: [public / local-auth / Entra SSO / admin-only]
- Email notification involved (Microsoft Graph): [yes/no]
